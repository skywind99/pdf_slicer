...
| ⏭️ 결번 건너뛰기 | 결석 등으로 빠진 번호를 지정하면 페이지 소비 없이 번호만 건너뜀 |
| 🔗 페이지 수 예외 | 특정 번호만 페이지 수가 다를 때(예: 답안 추가로 4쪽) 개별 지정 |
...
결석 등으로 답안지 자체가 없는 학생이 있거나, 특정 학생만 답안 페이지 수가 다를 때 사용합니다.

| 항목 | 입력 형식 | 동작 |
|---|---|---|
| ⑤ 결번(건너뛸 번호) | `3, 7, 15` (쉼표 구분) | 해당 번호는 파일을 만들지 않고 원본 페이지도 소비하지 않은 채 다음 번호로 넘어감 |
| ⑥ 페이지 수 예외 | `5:4, 12:6` (`번호:페이지수`, 쉼표 구분) | 해당 번호만 지정한 페이지 수만큼 합쳐서 하나의 파일로 저장, 이후 번호는 다시 기본 페이지 수로 진행 |

**예시** — 총 20쪽 PDF, 학생당 기본 2쪽, 결번 `3`, 페이지 수 예외 `5:4` 입력 시:

| 파일(번호) | 원본 쪽 범위 | 비고 |
|---|---|---|
| 01 | 1~2쪽 | 기본 2쪽 |
| 02 | 3~4쪽 | 기본 2쪽 |
| 03 | — | 결번으로 건너뜀(페이지 소비 없음) |
| 04 | 5~6쪽 | 기본 2쪽 |
| 05 | 7~10쪽 | 예외로 4쪽 합쳐짐 |
| 06 | 11~12쪽 | 다시 기본 2쪽으로 진행 |

> 같은 번호를 결번과 페이지 수 예외에 동시에 입력하면 오류로 처리됩니다.  
> 지정한 페이지 수가 남은 페이지보다 많으면 남은 페이지까지만 저장하고 완료 메시지로 경고합니다.
...
| v2.5 | 결번(건너뛸 번호), 페이지 수 예외(합칠 페이지) 기능 추가 |
